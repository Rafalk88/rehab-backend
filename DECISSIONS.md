...
## Decision 013 - Automated Audit Logging via Prisma Middleware

- **Date**: 2025-11-01
- **Status**: In Testing
- **Context**:
  Current audit logging was implemented by overriding `Prisma` CRUD methods in a custom singleton wrapper.
  While this ensured that every operation automatically produced an audit entry, the approach had several drawbacks:
  - Risk of incompatibility after `Prisma` updates (since internal method contracts may change).
  - Difficulties in unit testing (mocked `Prisma` instances bypass patched methods).
  - Lack of flexibility for non-CRUD operations and limited separation of concerns.
  - Dependency on a global context object that required additional logic for isolation.
    Over the last week, the architecture was refactored and experimentally tested to move toward a middleware-based auditing system.
- **Decision**:
  Replace method overriding with an official `Prisma` `$use` middleware that automatically logs all CRUD operations.
  - The middleware will use `AsyncLocalStorage` to isolate request context (e.g., `userId`, `IP`, `requestId`).
  - Audit entries are written transparently after each database modification (create, update, delete).
  - Non-CRUD actions (custom operations, reports, etc.) will use a shared `logAudit()` helper to maintain consistency.
  - The global `Prisma` singleton remains, but it no longer patches internal methods.
- **Rationale**:
  - Higher stability: $use is a public `Prisma` API, safe for upgrades.
  - Improved testability: audit middleware can be mocked or disabled during unit tests.
  - Simpler context handling: `AsyncLocalStorage` provides per-request scoping without global mutation.
  - Future-proof: centralized, declarative logic compatible with any future `Prisma` extensions.
- **Status / Notes**:
